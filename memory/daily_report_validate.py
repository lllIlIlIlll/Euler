...
# 复用 render 的板块定义
try:
    from daily_report_render import SECTIONS, parse_date
except ImportError:
    SECTIONS = [
...
def check_e4_07_timeliness(data: dict) -> Tuple[bool, List[str]]:
    ...
    for i, item in enumerate(all_items):
        ...
        if m:
            item_date = datetime(rd.year, int(m.group(1)), int(m.group(2)))
            delta = (rd - item_date).days
            if delta > 7:
                errors.append(f"条目{i+1}: '{pd}' 超出7天时效窗口 ({delta}天)")
    return len(errors) == 0, errors
    ...
    for code in ["s1", "s2"]:
        ...
        for i, item in enumerate(items, 1):
            ...
            if body and not pattern.match(body):
                errors.append(f"{code.upper()} 条目{i}: 正文开头未匹配段首格式 '{body[:30]}...'")
    for sub in ["s3_hot", "s3_clues"]:
        items = data.get(sub, [])
        for i, item in enumerate(items, 1):
            body = item.get("body", "")
            if body and not pattern.match(body):
                errors.append(f"{sub.upper()} 条目{i}: 正文开头未匹配段首格式 '{body[:30]}...'")
    return len(errors) == 0, errors
