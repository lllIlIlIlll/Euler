...
SECTIONS = [
    ...
    {"code": "S2", "num": "二", "title": "主要国家及国际组织的重要动向",
     "subtitle": None, "count_range": (0, 10)},
        ...
        if sec_def.get("is_analysis"):
            ...
            if isinstance(trends, dict):
                sub_items = [
                    ...
                    ("**涉华影响方向：**", trends.get("china_impact", "")),
                ]
...
def _strip_body_lead(body: str) -> str:
    ...
    import re as _re
    return _re.sub(r'^\d{1,2}月\d{1,2}日.{1,20}(报道|表示|声明|发布)：', '', body)
...
def main():
    ...
    parser.add_argument("--fmt", choices=["md", "docx", "html", "all"], default="all",
                        help="输出格式 (默认 all)")
